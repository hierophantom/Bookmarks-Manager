...
function createSearchField(options = {}) {
  ...
  container.className = `search-field search-field--${contrast}`;
  ...
  const setHasValue = (hasValue) => {
    container.classList.toggle('search-field--has-value', hasValue);
  };

  if (disabled) {
    input.disabled = true;
    container.classList.add('search-field--disabled');
  }
  ...
  if (shortcutKeys && shortcutKeys.length > 0) {
    ...
    shortcutEl.className = 'search-field__shortcut';
    shortcutEl.setAttribute('aria-hidden', 'true');

    shortcutKeys.forEach(key => {
      const keyEl = document.createElement('span');
      keyEl.className = 'search-field__shortcut-key';
  ...
  container.addEventListener('click', (event) => {
    if (disabled) return;
    if (event.target.closest('.search-field__clear')) return;
    input.focus();
  });
...
function updateSearchCompValue(searchComp, value) {
  const input = searchComp.querySelector('.search-field__input');
  if (input) {
    input.value = value;
  }
  searchComp.classList.toggle('search-field--has-value', Boolean(value));
}
...
function toggleSearchCompDisabled(searchComp, disabled) {
  ...
  searchComp.classList.toggle('search-field--disabled', disabled);
}
...
function updateSearchCompShortcut(searchComp, keys) {
  const existingShortcut = searchComp.querySelector('.search-field__shortcut');
  if (existingShortcut) {
  ...
  if (keys && keys.length > 0) {
    ...
    shortcut.className = 'search-field__shortcut';
...
function updateSearchCompContrast(searchComp, contrast) {
  searchComp.classList.remove('search-field--low', 'search-field--high');
  searchComp.classList.add(`search-field--${contrast}`);
}
