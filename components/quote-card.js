 ...
 * const quote = createQuoteCardCardCard({
 ...
 * const shortQuote = createQuoteCardCardCard({
 *   text: 'Be yourself.',
 *   author: 'Oscar Wilde',
 *   maxWidth: 300,
 ...
 */
function createQuoteCardCardCard(options = {}) {
  const {
    text = '',
    author = '',
    onAuthorClick = null,
  ...
  container.className = 'quote-card';
  
  if (noTruncate) {
    container.classList.add('quote-card--no-truncate');
  }

  // Create quote text element
  const quoteText = document.createElement('span');
  quoteText.className = 'quote-card__text';
  ...
  sep.className = 'quote-card__separator';
  sep.textContent = separator;
  container.appendChild(sep);

  // Create author button
  const authorBtn = document.createElement('button');
  authorBtn.className = 'quote-card__author';
...
function updateQuoteText(quoteElement, text) {
  const textEl = quoteElement.querySelector('.quote-card__text');
  if (textEl) {
...
function updateQuoteAuthor(quoteElement, author) {
  const authorEl = quoteElement.querySelector('.quote-card__author');
  if (authorEl) {
...
function toggleQuoteTruncation(quoteElement, noTruncate) {
  quoteElement.classList.toggle('quote-card--no-truncate', noTruncate);
}

// Export for module usage
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    createQuoteCardCardCard,
    updateQuoteText,
    updateQuoteAuthor,
    updateQuote,
    setQuoteMaxWidth,
