...
/**
 * Creates a bookmark-gallery-tile-small element
 ...
 */
function createBookmarkGalleryTileSmall(options = {}) {
  const {
    type = 'folder',
    state = 'idle',
    label = 'Label',
  ...
  view.className = 'bookmark-gallery-tile-small';

  applyBookmarksGalleryViewType(view, type);
  applyBookmarksGalleryViewState(view, state);

  const content = document.createElement('div');
  content.className = 'bookmark-gallery-tile-small__content';

  const iconValue = icon || (type === 'bookmark' ? 'bookmark' : 'folder');
  const iconEl = createBookmarkGalleryTileSmallIcon(iconValue);
  if (iconEl) {
    content.appendChild(iconEl);
  }

  const textEl = document.createElement('div');
  textEl.className = 'bookmark-gallery-tile-small__text';

  const labelEl = document.createElement('div');
  labelEl.className = 'bookmark-gallery-tile-small__label';
  labelEl.textContent = label;

  const subtextEl = document.createElement('div');
  subtextEl.className = 'bookmark-gallery-tile-small__subtext';
  ...
  if (actionsToRender.length > 0) {
    ...
    actionsEl.className = 'bookmark-gallery-tile-small__actions';
    ...
    if (state === 'idle') {
      ...
      if (shouldShow) {
        view.classList.add('bookmark-gallery-tile-small--actions-visible');
      }
    }
  }
...
/**
 * Applies bookmark-gallery-tile-small type
...
function applyBookmarksGalleryViewType(view, type) {
  ...
  view.classList.remove('bookmark-gallery-tile-small--bookmark', 'bookmark-gallery-tile-small--folder');
  view.classList.add(`bookmark-gallery-tile-small--${type}`);
}

/**
 * Applies bookmark-gallery-tile-small state
 * @param {HTMLDivElement} view - View element
 * @param {string} state - 'idle' or 'hover'
 */
function applyBookmarksGalleryViewState(view, state) {
  if (!view || !view.classList) return;
  view.classList.remove('bookmark-gallery-tile-small--idle', 'bookmark-gallery-tile-small--hover');
  view.classList.add(`bookmark-gallery-tile-small--${state}`);
}
...
function updateBookmarksGalleryViewLabel(view, label) {
  const labelEl = view.querySelector('.bookmark-gallery-tile-small__label');
  if (labelEl) {
    labelEl.textContent = label;
  }
}
...
function updateBookmarksGalleryViewSubtext(view, subtext) {
  const subtextEl = view.querySelector('.bookmark-gallery-tile-small__subtext');
  if (subtextEl) {
...
function createBookmarkGalleryTileSmallIcon(icon) {
  if (!icon) return null;

  const iconEl = document.createElement('div');
  iconEl.className = 'bookmark-gallery-tile-small__icon';
...
function getDefaultBookmarksGalleryViewActions(type, state) {
  ...
  const labelAction = createIconButton({
    icon: 'label',
    label: 'Label',
    tooltip: 'Label'
  });
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    createBookmarkGalleryTileSmall,
