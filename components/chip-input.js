...
function createChipInput(options = {}) {
  const {
    label = 'Label',
    contrast = 'low',
    state = 'idle',
  ...
  field.className = `chip-input chip-input--${contrast}`;

  const content = document.createElement('div');
  content.className = 'chip-input__content';

  const chipList = document.createElement('div');
  chipList.className = 'chip-input__chip-list';
  ...
  action.className = 'chip-input__action';
  action.type = 'button';
  ...
  menuWrapper.className = 'chip-input__menu';
  ...
  const applyState = () => {
    field.classList.toggle('chip-input--active', active);
    field.classList.toggle('chip-input--hover', !active && state === 'hover');
    field.classList.toggle('chip-input--has-value', selection.length > 0);
    field.classList.toggle('chip-input--has-query', Boolean(query));
  };
  ...
  const openMenu = () => {
    active = true;
    menuWrapper.classList.add('chip-input__menu--open');
    applyState();
    renderMenu();
  };
  ...
  const closeMenu = () => {
    active = false;
    menuWrapper.classList.remove('chip-input__menu--open');
    applyState();
  };
