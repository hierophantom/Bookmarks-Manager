...
function createSegmentedControlsss(options = {}) {
  const {
    items = [],
  ...
  control.className = [
    'segmented-controls',
    `segmented-controls--${contrast}`,
    disabled ? 'segmented-controls--disabled' : '',
    className
  ].filter(Boolean).join(' ');
  ...
  if (showTitle) {
    ...
    titleEl.className = 'segmented-controls__title';
    titleEl.textContent = title;
    control.appendChild(titleEl);
  }

  const surface = document.createElement('div');
  surface.className = 'segmented-controls__surface';
...
function renderSegmentedControlItems(control) {
  ...
  const surface = control?.querySelector('.segmented-controls__surface');
  ...
  config.items.forEach((item, index) => {
    ...
    button.type = 'button';
    button.className = 'segmented-controls__item';
    ...
    button.classList.toggle('segmented-controls__item--selected', selected);

    if (index === config.items.length - 1) {
      button.classList.add('segmented-controls__item--last');
    }

    if (config.disabled || item.disabled) {
      button.disabled = true;
      button.classList.add('segmented-controls__item--disabled');
    }

    const labelEl = document.createElement('span');
    labelEl.className = 'segmented-controls__item-label';
...
function updateSegmentedControlTitle(control, title) {
  const titleEl = control?.querySelector('.segmented-controls__title');
  if (titleEl) {
    titleEl.textContent = title;
  }
}
...
function setSegmentedControlDisabled(control, disabled) {
  ...
  control.classList.toggle('segmented-controls--disabled', Boolean(disabled));
  const surface = control.querySelector('.segmented-controls__surface');
  if (surface) {
    if (disabled) {
      surface.setAttribute('aria-disabled', 'true');
