 ...
 *
 * @example
 * const sessionsPanel = createActiveSessionPanel({
 *   docked: true,
 ...
 */
function createActiveSessionPanel(options = {}) {
  const {
    docked = false,
    onClose = null,
    onToggleMode = null,
  ...
  panel.element.classList.add('active-session-panel');
  panel.content.classList.add('active-session-panel__content');
  ...
  function createTabItem(tab, handlers = {}) {
    ...
    tabItem.className = 'active-session-panel__tab-item';
    ...
    if (typeof FaviconService !== 'undefined' && tabUrl) {
      const favicon = FaviconService.createFaviconElement(tabUrl, {
        size: 24,
        className: 'active-session-panel__tab-favicon',
        alt: 'Site icon'
      });
      tabItem.appendChild(favicon);
    } else {
      const fallbackIcon = document.createElement('span');
      fallbackIcon.className = 'active-session-panel__tab-favicon material-symbols-outlined';
    ...
    textWrap.className = 'active-session-panel__tab-text';

    const titleEl = document.createElement('span');
    titleEl.className = 'active-session-panel__tab-title';
    titleEl.setAttribute('dir', 'auto');
    titleEl.textContent = tabTitle;
    textWrap.appendChild(titleEl);

    const urlEl = document.createElement('span');
    urlEl.className = 'active-session-panel__tab-url';
    ...
    actions.className = 'active-session-panel__tab-actions';
  ...
  return Object.assign(panel, {
    ...
    addWindowGroup(options = {}) {
      ...
      groupEl.className = 'active-session-panel__group';
      groupEl.setAttribute('data-window-id', String(id));

      const titleEl = document.createElement('h4');
      titleEl.className = 'active-session-panel__group-title';
      titleEl.textContent = label;

      if (incognito) {
        const maskIcon = document.createElement('span');
        maskIcon.className = 'active-session-panel__group-incognito material-symbols-outlined';
      ...
      tabsEl.className = 'active-session-panel__group-tabs';
      ...
      actionsWrap.className = 'active-session-panel__group-actions';

      let saveButton;
      if (typeof createPrimaryButton === 'function') {
        const icon = document.createElement('span');
        icon.className = 'material-symbols-outlined active-session-panel__save-icon';
      ...
      saveButton.classList.add('active-session-panel__save-btn');
      actionsWrap.appendChild(saveButton);

      let exportButton;
      if (typeof createCommonButton === 'function') {
        const icon = document.createElement('span');
        icon.className = 'material-symbols-outlined active-session-panel__export-icon';
      ...
      exportButton.classList.add('active-session-panel__export-btn');
    ...
    addSession(options = {}) {
      ...
      item.className = 'active-session-panel__item';
      item.setAttribute('data-session-id', id);

      // Session header
      const header = document.createElement('div');
      header.className = 'active-session-panel__item-header';

      // Label
      const labelEl = document.createElement('span');
      labelEl.className = 'active-session-panel__item-label';
      labelEl.textContent = label;
      header.appendChild(labelEl);

      // Controls
      const controls = document.createElement('div');
      controls.className = 'active-session-panel__item-controls';

      // View button
      const viewBtn = document.createElement('button');
      viewBtn.className = 'active-session-panel__item-btn active-session-panel__item-btn--view';
      viewBtn.type = 'button';
      ...
      removeBtn.className = 'active-session-panel__item-btn active-session-panel__item-btn--remove';
    ...
    updateSession(id, updates) {
      ...
      if (updates.label !== undefined) {
        const labelEl = element.querySelector('.active-session-panel__item-label');
        if (labelEl) {
