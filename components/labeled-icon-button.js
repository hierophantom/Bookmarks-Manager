 ...
 * const button = createLabeledIconButton({
 *   icon: '+',
 *   label: 'Add Item',
 *   colorScheme: 'primary',
 ...
 */
function createLabeledIconButton(options = {}) {
  const {
    icon = '',
    label = '',
    onClick = null,
  ...
  button.className = `labeled-icon-button labeled-icon-button--${colorScheme} labeled-icon-button--${size}`;
  button.setAttribute('type', 'button');
  
  if (!label && icon) {
    button.classList.add('labeled-icon-button--icon-only');
  }
  ...
  if (icon) {
    ...
    iconEl.className = 'labeled-icon-button__icon';
  ...
  if (label) {
    ...
    labelEl.className = 'labeled-icon-button__label';
...
function updateCubeActionButtonWithLabelText(button, label) {
  const labelEl = button.querySelector('.labeled-icon-button__label');
  if (labelEl) {
    labelEl.textContent = label;
    button.setAttribute('aria-label', label);
  }
...
function updateCubeActionButtonWithLabelIcon(button, icon) {
  const iconEl = button.querySelector('.labeled-icon-button__icon');
  if (!iconEl) return;
  
  iconEl.innerHTML = '';
  
  const size = button.classList.contains('labeled-icon-button--medium') ? 'medium' : 'small';
...
function updateCubeActionButtonWithLabelColorScheme(button, colorScheme) {
  button.classList.remove(
    'labeled-icon-button--primary',
    'labeled-icon-button--default',
    'labeled-icon-button--destructive'
  );
  button.classList.add(`labeled-icon-button--${colorScheme}`);
}
